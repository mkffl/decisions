...
class SteppyCurve(
  ...
  def bayesErrorRate = PP.transpose * pMisspFa.transpose map { case e: Row =>
    e reduceLeft (_ + _)
  }
...
class PAV(
  ...
  private def countTargets: Vector[Int] =
    pavFit.bins.map { case po: Point =>
      po.getWeight.toDouble * po.getY toInt
    }.toVector

  private def countNonTargets: Vector[Int] =
    targets
      .zip(pavFit.bins)
      .map { case (count, po) => (po.getWeight - count).toInt }
      .toVector
  ...
  def EER: Double = {
    ...
    val maximised = new EvalUtils.BrentOptimizerScalarWrapper(
      objectiveFunction,
      priorLogOdds(0),
      priorLogOdds.last,
      minimize = false
  ...
  def scoreVSlogodds: Tuple2[Row, Row] = pavFit.bins.map { case po =>
    (po.getX, logit(po.getY))
  }.unzip
}
...
object To {
  ...
  def makeHisTo(
    ...
    val thresh =
      histogram(tarPreds, numBins, min, max).map(_._1.toDouble).toVector
    ...
    def isocost(pa: AppParameters): Segment = {
      ...
      val (bfpr, btpr) =
        (
          roc(0).reverse(ii + 1),
          roc(1).reverse(ii + 1)
        ) // .reverse because roc curves are score inverted
      val (x1, x2) = (roc(0)(0), roc(0).last)
      val (y1, y2) =
        (affine(btpr, bfpr, x1, slope), affine(btpr, bfpr, x2, slope))
...
object Concordance {
  ...
  def rankAvgTies(input: Row): Row = {
    ...
    val enhanced = input.zipWithIndex.sortBy(_._1).zipWithIndex.map {
      case ((lo, index), rank) => Rank(lo, index, rank + 1)
    }
    val avgTies = enhanced.groupBy(_.value).map { case (value, v) =>
      (value, v.map(_.rank.toDouble).sum / v.map(_.rank).size.toDouble)
    }
    val joined = for {
      e <- enhanced
      a <- avgTies
...
object EvalUtils {
   ...
   */
  class PairAdjacentViolatorsWrapper(
  ...
  class BrentOptimizerScalarWrapper(
    ...
    val interval = new SearchInterval(
      intervalMin,
      intervalMax,
      0.5 * (intervalMax - intervalMin)
    )
  ...
  case class Reliability(
      bin: Double,
      frequency: Double,
      accuracy: Double,
      count: Integer
  )
  val avg: (Array[Double] => Double) = values =>
    values.sum / values.size.toDouble
  val binTheValue: (Double => (Double => Double)) = width =>
    (i => round(i / width).toInt * width)
  ...
  def binnedAccuracy(
      pDev: Array[Double],
      yDev: Array[Int],
      binValue: Double => Double
  ): Seq[Reliability] = {
    val sorted: Array[(Double, Int)] = pDev.zip(yDev).sorted
    val binned = sorted.map { case (proba, label) =>
      (binValue(proba), proba, label)
    }

    val bins = binned.groupMap(_._1)(_._3).map { case (bin, label) => bin }
    val counts =
      binned.groupMap(_._1)(_._3).map { case (bin, label) => label.size }
    val accuracy = binned.groupMap(_._1)(_._3).map { case (bin, label) =>
      avg(label.map(_.toDouble))
    }
    val frequency =
      binned.groupMap(_._1)(_._2).map { case (bin, proba) => avg(proba) }
  ...
  def ECE(data: Seq[Reliability]): Double = {
    ...
    val ece = data
      .map { bin => (abs(bin.accuracy - bin.frequency)) * (bin.count / N) }
      .reduce(_ + _)
    ece
  }
  ...
  def minRiskBruteForce(
    ...
    val priorCosts: Matrix = for (row <- PP) yield row match {
      ...
      case _ =>
        Row(-10, -10) // Something's off, insert crazy values to flag the issue
    }
   ...
   */
  def paramToθ(pa: AppParameters): Double = log(
    pa.p_w1 / (1 - pa.p_w1) * (pa.Cmiss / pa.Cfa)
  )

  /*
        RR = p_w0*Cfa/p_w1*Cmiss
   */
  def paramToRiskRatio(p: AppParameters): Double =
    ((1 - p.p_w1) * p.Cfa) / (p.p_w1 * p.Cmiss)
  ...
  def affine(y1: Double, x1: Double, x2: Double, slope: Double) =
    y1 + (x2 - x1) * slope
